...
Every trusted extracted numeric value carries exact source text, 1-based page, and reconstructed 1-based line. Preserve available row context. Do not infer missing numbers. Keep valid rows when another row is uncertain. Classify table rows as items on every page. All other visible text becomes page-sourced details: `Label: value` rows use their label and unlabeled rows use `Text`. Collapse identical values only within their page. Use local OCR only on pages without a text layer and leave uncertain OCR values empty in error-bearing item rows.

## Implemented milestones

1. **Workspace and contract:** Bun workspaces for the Hono API, React/Vite app, and shared Zod types.
2. **PDF extraction:** PDF.js positioned text, row grouping, header-based column detection, local OCR fallback for textless pages, row arithmetic checks, inline item errors, and page-sourced details. All pages are checked for item tables; uncertain OCR rows remain visible as error-bearing items. Document-level reconciliation is omitted.
