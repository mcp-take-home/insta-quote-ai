...
The upload endpoint saves the file and queued record, then returns `202` without parsing it in the request. A worker in the API process claims queued records through Drizzle, reads each PDF, and persists either a completed result or a processing failure. The web app polls the document endpoint about once a second and stops when it reaches a terminal state. Every page is checked for table rows; those become `items` regardless of page heading. Other visible text becomes page-sourced `details`, including repeated headers and totals on their respective pages. Identical text is collapsed only within its page. New results include `pageCount` so empty pages retain a page section; older saved results still load. Textless pages use local OCR to identify rows; OCR-derived numbers stay untrusted and the rows appear with subtle errors for review. Row errors stay on their table rows. Page and line references use 1-based numbering; a line is one reconstructed visible text row on that PDF page, including separator rows.
